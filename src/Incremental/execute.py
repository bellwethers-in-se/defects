...
def bellw_incr(source, target, n_rep=10):
    ...
    for src_name, src_path in source.iteritems():
        if src_name == "lucene":
            for n in xrange(1, len(src_path.data)+1):
                ...
                name = src_data[-1].split('/')[-1].split('.csv')[0]  # Get the name of the bellwether proj
                ...
                stats = pandas.DataFrame(sorted(stats_lst, key=lambda lst: lst[-2], reverse=True),  # Sort by G Score
